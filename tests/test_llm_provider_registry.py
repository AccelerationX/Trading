                ...
                """
{
  "providers": [
    ...
    def test_trade_plan_prefers_remote_provider_when_credentials_ready(self) -> None:
        packets = [
            LLMWorkPacket(
                packet_id="p1",
                trade_date="2026-05-06",
        ...
        routes = resolve_llm_execution_routes(
            packets,
            providers,
            environ={"MOONSHOT_API_KEY": "x", "MOONSHOT_BASE_URL": "https://api.moonshot.cn/v1"},
        )
        self.assertEqual(routes[0].provider_id, "moonshot_kimi_primary")
        self.assertIn("routing_policy=remote_core", routes[0].notes)

    def test_candidate_diagnosis_prefers_remote_provider_when_credentials_ready(self) -> None:
        packets = [
            LLMWorkPacket(
                packet_id="p1",
                trade_date="2026-05-06",
                agent_id="candidate_diagnosis_agent",
                task_id="candidate_diagnosis",
                priority="high",
                target_object_type="candidate_card",
                target_object_id="candidate1",
                prompt_file="prompts/tasks/candidate_diagnosis.md",
                expected_output_contract="candidate_card_diagnosis",
            )
        ]
        providers = [
            LLMProviderConfig(
                ...
                supports_json_mode=True,
                agent_overrides=(),
                agent_allowlist=("candidate_diagnosis_agent",),
            ),
            LLMProviderConfig(
                provider_id="moonshot_kimi_primary",
                enabled=True,
