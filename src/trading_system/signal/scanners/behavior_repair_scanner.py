...
from trading_system.signal.legacy.behavior_repair_core import scan_behavior_repair_candidates
...
class BehaviorRepairScanner:
    """TM101: Behavior repair and rebound-state candidate scanner."""
    ...
    def module_id(self) -> str:
        return "TM101_behavior_repair_rebound"
    ...
    def scan(
        ...
        df = self._ensure_loaded()
        candidates = scan_behavior_repair_candidates(
            df,
        ...
        for _, row in candidates.iterrows():
            ...
            confidence = min(0.95, 0.55 + float(row.get("signal", 0)) * 0.10)
            ...
            signals.append(
                ModuleSignal(
                    ...
                    strength=strength,
                    technical_state=row.get("technical_state", "repair_unknown"),
                    confidence=confidence,
                    metadata={
                        "signal_value": float(row.get("signal", 0)),
                        "repair_accel_signal": float(row.get("repair_accel_signal", 0)) if pd.notna(row.get("repair_accel_signal")) else None,
                        "trend_repair_signal": float(row.get("trend_repair_signal", 0)) if pd.notna(row.get("trend_repair_signal")) else None,
                    ...
                    invalidation_hint="Rebound momentum fades or volatility spikes above median",
                    source_refs=["behavior_repair_core.py::scan_behavior_repair_candidates"],
                )
            )
        return signals
...
register_scanner("TM101_behavior_repair_rebound", _factory)
