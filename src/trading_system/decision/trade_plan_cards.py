...
def _position_multiplier(snapshot: MarketRegimeSnapshot, card: CandidateCard) -> float:
    ...
    score = card.candidate_score if card.candidate_score is not None else 0.0
    if score >= 0.8:
        base *= 1.0
    elif score >= 0.7:
        base *= 0.8
...
def _position_size_rule(account: AccountConstraints, snapshot: MarketRegimeSnapshot, card: CandidateCard) -> tuple[str, float | None]:
    ...
    max_position_pct = round(account.single_position_max_pct * multiplier, 4)
    initial_pct = round(max_position_pct * 0.5, 4)
...
def build_trade_plan_cards(
    ...
    for rank, card in enumerate(eligible_candidates, start=1):
        ...
        if has_severe_flag:
            ...
            action = "watch_only"
        elif rank <= action_slots and score >= 0.6:
            action = "buy_pilot"
        else:
            action = "watch_only"
